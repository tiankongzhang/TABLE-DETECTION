...
def resize_image(im, max_side_len=2400):
    ...
    #print([int(resize_w), int(resize_h)])
...
def main(argv=None):
    ...
    isExists=os.path.exists(FLAGS.output_dir)
    if not isExists:
                ...
                if im is not None and im.shape[0]>=32 and im.shape[1]>=32:
                    ...
                    if boxes is not None:
                        ...
                        for box in boxes:
                            ...
                            coord_index += 1
                            cv2.polylines(bim, [box.astype(np.int32).reshape((-1, 1, 2))], True, color=(255, 255, 0), thickness=1)
                        
                        txt_id = os.path.basename(im_fn).split('.')[0]
                        
                        outputs_dir = os.path.join(FLAGS.output_dir, 'jsondata')
